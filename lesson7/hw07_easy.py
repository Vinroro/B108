...
i = input("Enter a point number(0, 1 or 2) to get its height value: ")
...
class Three_equal_sides_trapezoid:
    ...
    def three_equal_sides_check(self):
        no_unique_side_lengths = self.lengths()
    ...
    def square(self):
        if self.three_equal_sides_check():
            ...
            uniqueSide = max(self.lengths())
            perimeter_half = self.perimeter() / 2

            return ((perimeter_half - uniqueSide) * (perimeter_half - side) ** 3) ** (1 / 2)
        else:
...
print(f"Parameters of trapezoid with points {figure.points[0]}, {figure.points[1]}, {figure.points[2]}, {figure.points[3]}")
